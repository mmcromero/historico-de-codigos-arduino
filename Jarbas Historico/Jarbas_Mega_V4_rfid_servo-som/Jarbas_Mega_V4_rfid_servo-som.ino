...
void verificaPorta()
{
  if(!digitalRead(SM)){ //sensor aberto
    // desabilita tranca da porta
    FLAGTrancaPorta = 2;
    // desabilita sensor de presença
    if(FLAGPirHall == 1){FLAGPirHall = 0;}
    //Mantem luz ligada
    if(STATUSRele1 == 0){ luzHall(); }
  
  }else{ //sensor fechado
    // Habilita tranca da porta
    if(FLAGTrancaPorta == 2){FLAGTrancaPorta = 0; }
    //habilita Pir
    if(FLAGPirHall == 0){FLAGPirHall = 1; }
  }

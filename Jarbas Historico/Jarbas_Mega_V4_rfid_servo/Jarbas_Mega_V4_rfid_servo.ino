...
int const servoMotorPin1    = 47; 
int const servoMotorPin2    = 48; 
...
void setup(){ 
  ...
  Serial.println("Encoste o chaveiro ou cartão...");
  ...
  servoMotorObj1.write(180);
  
  
  servoMotorObj2.attach(servoMotorPin2); 
  servoMotorObj2.write(180); //aqui o motor irá "fechar"
  //delay(1650); 
  delay(2250); 
  servoMotorObj2.write(0); //aqui o motor irá "abrir"
  delay(700); 
  servoMotorObj2.write(70); //aqui o motor irá "parar"
...
void loop(){
  ...
  if (cmd.length() >0) { 
    ...
    if(cmd == "servo1 abre fecha"){ servo1(1);}
    if(cmd == "servo1 abre"){ servo1(2);}
    if(cmd == "servo1 fecha"){ servo1(3);}
  ...
  // Prepare key - all keys are set to FFFFFFFFFFFFh at chip delivery from the factory.
        ...
        for (byte i = 0; i < mfrc522.uid.size; i++) {
          ...
          //k.keyByte[] = mfrc522.uid.uidByte[i];
        } 
...
void servo1(int estado)
{   
  if(estado == 1){
    servoMotorObj1.write(2);
    delay(500);
    servoMotorObj1.write(180); 
  }
  if(estado == 2){servoMotorObj1.write(2); }
  if(estado == 3){servoMotorObj1.write(180); }

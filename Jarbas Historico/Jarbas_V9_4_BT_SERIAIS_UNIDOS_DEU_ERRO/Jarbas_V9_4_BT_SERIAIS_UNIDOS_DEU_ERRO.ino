...
String cmd ; // input serial
String readString; // input serial
String readString1; // input serial
String readString2; // input serial
char strLocal      = ' '; 
//String valor ; // input serial
...
void setup(){ 
  ...
  //configuraModulo(valor);         //chamando a função para configurar o módulo.
...
void loop(){
  ...
  if (cmd.length() >0) { 
        if(cmd == "oi"){Serial.println("Ola");}
        if(cmd == "tudo bem" || cmd == "tudo bem?" || cmd == "tudo bem ?"){Serial.println("Tudo e vc ?");}
              
        if(cmd == "temp ext"){tempExterno();}
        if(cmd == "temp int"){tempInterno();}
        if(cmd == "temp arthur"){tempInterno();}
        ...
        if(cmd == "porta on") {flagSM = 1; Serial.println("Sensor Porta Ligado");}
        if(cmd == "porta off"){flagSM = 0; Serial.println("Sensor Porta Desligado");}
        
        if(cmd == "geral on"){FLAGChaveGeral = OFF; chaveGeral();}
        if(cmd == "geral off"){FLAGChaveGeral = ON; chaveGeral();}
        
        if(cmd == "s+"){Serial.println("sobe 1 segundo"); configuraRTCSegMais();}
        if(cmd == "s-"){Serial.println("desce 1 segundo"); configuraRTCSegMenos();}
        
        if(cmd == "m+"){Serial.println("sobe 1 minuto"); configuraRTCMinMais();}
        if(cmd == "m-"){Serial.println("desce 1 minuto"); configuraRTCMinMenos();}
        
        if(cmd == "h+"){Serial.println("sobe 1 hora"); configuraRTCHoraMais();}
        if(cmd == "h-"){Serial.println("desce 1 hora"); configuraRTCHoraMenos();}
...
void configuraRTCSegMais()
{
  ...
  segundo = bcdToDec(Wire.read() & 0x7f);                                     //bits de controle. 

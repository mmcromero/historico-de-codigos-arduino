String jarbasVersion = " Jarbas V-7.7.3 ";
...
// flags para botoes do controle remote com dupla ação
...
/* ------------------------------------------------------------------ OSC ------------------------------------------------------------------ */
  ...
  //serialPrint("teste", false);
  //serialPrint("teste", true);
  //serialPrint("teste na outra linha", false);
...
void loop(){
  ...
  if(serverOsc.availableCheck()>0){/*Serial.println("OSC");*/}// deixar essa linha para que o osc funcione
...
//network.read(forA,&message,sizeof(unsigned long));

//network.read( &ReceivePackage, sizeof(ReceivePackage) );
//network.read(forA,comandos,sizeof(comandos));
//Serial.print("Retorno nrf24 - node 1 - ");
